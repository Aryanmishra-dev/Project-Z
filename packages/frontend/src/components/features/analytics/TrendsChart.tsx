    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-lg">
        ...
        <p className="text-xs text-gray-500">
          {data?.quizzes} {data?.quizzes === 1 ? 'quiz' : 'quizzes'} • {data?.totalQuestions} questions
        </p>
      </div>
    );
  }
...
const TrendIndicator = ({ trend, rate }: { trend: string; rate: number }) => {
  ...
  return (
    <div className={cn('flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium', colors[trend as keyof typeof colors])}>
      ...
      <CardContent>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          {(['easy', 'medium', 'hard'] as const).map((difficulty) => {
            ...
            const accuracy = perfData.totalQuestions > 0 
